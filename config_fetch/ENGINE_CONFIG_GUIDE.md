...
Keep EC2 provisioning separate from config capture:

```bash
# Create/start the GPU instance and verify SSH, Docker, GPU, and disk.
config_fetch/provision_aws_gpu_capture.sh

# Run capture on the already-running instance.
config_fetch/run_aws_gpu_capture.sh --skip-pull
```

`provision_aws_gpu_capture.sh` is the only script that runs Terraform. It
creates or starts the managed GPU instance and then exits.
`run_aws_gpu_capture.sh` does not run Terraform; it only syncs this repo to the
running instance, executes `capture_dynamo_runtime_configs.sh`, copies
`config_fetch/dynamo_runtime/` back locally, and stops the instance after a
successful run. If capture fails, the instance is left running for inspection.
...
the `ui` tier metadata and orders each engine catalog by frequency-of-use
buckets. Worker-specific prefill/decode files are not generated by this flow.
...
| Field | Meaning |
| --- | --- |
| `display_name` | Human-friendly display value for UI forms, for example `Router Mode` for `router_mode`. |
...
```json
{
  ...
  "DYN_SGL_DISAGG_CONFIG": {
    "display_name": "DYN SGL Disagg Config",
...
| Field | Meaning |
| --- | --- |
| `display_name` | Human-friendly label for UI forms. Env acronyms should remain readable, for example `NCCL IB Disable`. |
...
2. Use `display_name` as the visible field label. Keep the raw arg/env key for
   payloads, validation, and search, not as the primary form label.
3. Show descriptions as tooltips/help text, not always-on table content.
4. Show `ui: primary` in the main profile form.
5. Put `ui: advanced` behind an advanced section.
6. Keep `ui: less_frequent` searchable or available in "show all".
7. If a record has `choices` or `enum`, render a dropdown and reject values
   outside that set.
8. Never delete records from the catalog to simplify the UI.
...
Regenerate per-engine bucket ordering without recapturing images:
...
6. Compare old/new JSONs for added, removed, and changed records. UI metadata
   changes can be reviewed separately because `ui`, `aic`, `display_name`, and
   `value` are not engine behavior.
